...
class CNN:
    ...
    def test_input_placeholder(self):
        t1 = tf.placeholder(tf.float32, shape=[self.batch_size, 224, 224, 3])
        t2 = tf.placeholder(tf.float32, shape=[self.batch_size, 224, 224, 3])
        t3 = tf.placeholder(tf.float32, shape=[self.batch_size, 224, 224, 3])
        t = tf.placeholder(tf.int64, shape=[self.batch_size, 2])
        return t1, t2, t3, t
...
def train(num_epochs):
    ...
    cnn = CNN(learning_rate=0.00001, batch_size=64)

    # get the train data stream
...
train_data_path = 'D:\\Wangze\\multi_input_cls\\train_data\\'
test_data_path = 'D:\\Wangze\\multi_input_cls\\test_data\\'
classes = ('cat', 'dog')
