        ...
        with tf.variable_scope("weights"):
            self.weights = {
                ...
                'conv3': tf.get_variable('conv3', [3, 3, 32, 64],
                                         initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'conv4': tf.get_variable('conv4', [3, 3, 64, 64],
                                         initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'conv5': tf.get_variable('conv5', [3, 3, 64, 128],
                                         initializer=tf.contrib.layers.xavier_initializer_conv2d()),
                'fc1': tf.get_variable('fc1', [7 * 7 * 128, 1024],
                                       initializer=tf.contrib.layers.xavier_initializer()),
                'fc2': tf.get_variable('fc2', [1024, 4],
                                       initializer=tf.contrib.layers.xavier_initializer()),
                }
        with tf.variable_scope("biases"):
            self.biases = {
                ...
                'conv3': tf.get_variable('conv3', [64, ],
                                         initializer=tf.constant_initializer(value=0.0, dtype=tf.float32)),
                'conv4': tf.get_variable('conv4', [64, ],
                                         initializer=tf.constant_initializer(value=0.0, dtype=tf.float32)),
                'conv5': tf.get_variable('conv5', [128, ],
                                         initializer=tf.constant_initializer(value=0.0, dtype=tf.float32)),
                'fc1': tf.get_variable('fc1', [1024, ],
                                       initializer=tf.constant_initializer(value=0.0, dtype=tf.float32)),
                'fc2': tf.get_variable('fc2', [4, ],
                                       initializer=tf.constant_initializer(value=0.0, dtype=tf.float32))
            }
...
def train(num_epochs):
    ...
    # get the train data
    train_image, train_label = read_and_decode("bearing_data\\train_data1797_07+14+21.tfrecords")
    train_batch_image, train_batch_label = get_batch(train_image, train_label, batch_size=net.batch_size)
    # get the test data
    test_image, test_label = read_and_decode("bearing_data\\test_data1797_07+14+21.tfrecords")
...
train_loss, train_accs, test_accs, time = train(num_epochs=800)
